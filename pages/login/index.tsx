      ...
      <Wrap>
        <Form>
          ...
          <LoginForm>
            <p>이메일</p>
            <EmailForm>
              <Email name="email" type="text" placeholder="이메일 선택" autoComplete="off" value={email} onChange={onChangeValues} />
              <span>@</span>
...
const VisibleIcon = styled.div<{ type: string }>`
  ...
  height: 22px;
  cursor: pointer;
  background: url("/images/login/password_visible.svg") no-repeat;
  ${({ type }) => type === "text" && ` background: url("/images/login/password_visible_white.svg") no-repeat;`}
`;
